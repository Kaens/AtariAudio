...
#if 0
// STReplay mode (all voices OFF) and Quartet mode (all voices ON) tables
// Measured and generated by Paulo Simoes on real hardware.

static const unsigned short STReplayTable[816] = 
{
...
static	void	InitYmTable(unsigned short *pOut,const unsigned short *pIn)
{
	const int maxValue = pIn[0];
	for (int a=15;a>=0;a--)
	{
		for (int b=a;b>=0;b--)
		{
			for (int c=b;c>=0;c--)
			{
				unsigned int data = *pIn++;

				data = (data * 32767) / maxValue;
...
uint16_t mixTable[16 * 16 * 16];
uint16_t mixTable5b[32 * 32 * 32];

void compute555Table()
{
	InitYmTable(mixTable, STReplayTable);
	...
	for (int a = 0; a < 16; a++)
	{
		for (int b = 0; b < 16; b++)
		{
			for (int c = 0; c < 16; c++)
			{
				for (int j = 0; j < 8; j++) // interpolate 8 combine
				{
					int v1 = mixTable[a * 256 + b * 16 + c];
					int a0 = (j & 1) ? a - 1 : a;
					int b0 = (j & 2) ? b - 1 : b;
					int c0 = (j & 4) ? c - 1 : c;
					a0 = (a0 < 0)?0:a0;
					b0 = (b0 < 0)?0:b0;
					c0 = (c0 < 0)?0:c0;
					int v0 = mixTable[a0 * 256 + b0 * 16 + c0];
					int vi;
					if (v0)
						vi = int(sqrtf(float(v0 * v1)));
					else
						vi = v1 / 2;

					int idx0 = ((a << 1)<<10) + ((b << 1)<<5) + (c << 1);

					int idx1 = ((a << 1) | ((j>>0) & 1))<<10;
					idx1 += ((b << 1) | ((j>>1) & 1))<<5;
					idx1 += ((c << 1) | ((j>>2) & 1))<<0;
	...
	for (int i = 0; i < 16; i++)
		printf("%04x,", mixTable[i]);
	printf("\n");
	for (int i = 0; i < 32; i++)
		printf("%04x,", mixTable5b[i]);
	printf("\n");

	for (int i=0;i<32*32*32;i++)
		if (mixTable5b[i] == 0xffff)
			printf("ERROR!! %d\n", i);
